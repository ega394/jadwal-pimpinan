...
export default function App(){
  ...
  const getVisible=()=>{
    if(tab==="tayang")return events.filter(e=>e.alur==="disetujui"&&(filterDate!=="range"?(!filterDate||filterDate==="week"?(()=>{const ws=weekStart(),we=weekEnd();return!filterDate||filterDate!=="week"||(e.tanggal>=ws&&e.tanggal<=we);})():e.tanggal===filterDate):(!filterFrom||e.tanggal>=filterFrom)&&(!filterTo||e.tanggal<=filterTo))).sort((a,b)=>(a.tanggal+a.jam).localeCompare(b.tanggal+b.jam));
    let base=events;
