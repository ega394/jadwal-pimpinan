...
export default function App(){
  ...
  const navGroups=[
    {label:"MENU UTAMA",items:[
      ...
      ...(role==="walikota"||role==="wakilwalikota"||role==="ajudan"||role==="timkom"?[{key:"jadwal",icon:"📅",label:"Jadwal Saya"},{key:"penugasan",icon:"🎯",label:"Penugasan Saya"}]:[]),
  ...
  const mobTabs=[
    ...
    ...(role==="walikota"||role==="wakilwalikota"||role==="ajudan"||role==="timkom"?[{key:"jadwal",label:"Jadwal",icon:"📅"},{key:"penugasan",label:"Tugas",icon:"🎯"}]:[]),
...
function PimpinanView({events, role, user, onDisposisi, onCatatanSave, setDelegTarget, upd, showT, isMobile}){
  ...
  const today = todayStr();
  const upcoming = myEvs.filter(e=>e.tanggal>=today&&new Date(e.tanggal+"T"+e.jam)>=now);
  const past = myEvs.filter(e=>new Date(e.tanggal+"T"+e.jam)<now).reverse();
  ...
  const title = role==="walikota"?"Wali Kota Tarakan":"Wakil Wali Kota Tarakan";
    ...
    <div style={{flex:1,overflowY:"auto",background:"#F4F7FF"}}>
      ...
      <div style={{background:"linear-gradient(135deg,"+NAVY+" 0%,#1B3360 100%)",padding:isMobile?"20px 18px 24px":"28px 36px 32px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{color:GOLD,fontSize:12,fontWeight:600}}>
          {fmt(todayStr())} &nbsp;·&nbsp; {upcoming.length} agenda akan datang
        </div>
      </div>

      <div style={{padding:isMobile?"14px":"20px 28px"}}>
        {/* Belum konfirmasi alert */}
        {upcoming.filter(e=>!statusWK(e)&&!e.delegasiKeWWK).length>0&&(
  ...
  const mainContentJSX=(<div style={{flex:1,minWidth:0,display:"flex",flexDirection:"column",background:"#F0F4FA",overflow:"hidden"}}>
    ...
    <div style={{flex:1,overflowY:"auto",padding:isMobile?"12px 14px calc(env(safe-area-inset-bottom,0px) + 72px)":"24px 32px 48px"}}>
      ...
      {showPenugasan
        ...
        :(role==="walikota"||role==="wakilwalikota")&&tab==="jadwal"
