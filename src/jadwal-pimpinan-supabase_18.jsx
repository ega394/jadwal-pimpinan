...
function loadUsers(){
  try{
    ...
    // Auto-migrate plaintext passwords ke hash (async, fire-and-forget)
    ...
    <div style={{background:"white",borderRadius:16,width:"100%",maxWidth:560,maxHeight:"90vh",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"16px 20px 20px"}}>
        ...
        {editUser&&<div style={{background:"#EBF0FA",borderRadius:12,padding:14,marginTop:8,border:"1.5px solid "+NAVY}}>
          ...
          <div style={{marginBottom:8}}><label style={{display:"block",fontSize:11,color:"#64748b",fontWeight:600,marginBottom:2}}>Password Baru (kosong = tidak ubah)</label><input type="password" onChange={e=>setEditUser(p=>({...p,_newPw:e.target.value||undefined}))} style={inp}/></div>
...
export default function App(){
  ...
  const doBioLogin=async()=>{
    ...
    try{await bioAuthenticate(un);setUser(u);setTab("jadwal");try{localStorage.setItem("jp_session",JSON.stringify({username:un}));}catch{}registerPush(un,u.role);}
