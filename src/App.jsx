...
const ALL_ROLE_DEFS=[
  ...
  {key:"kasubbag_komdokpim", label:"Kasubbag Komdokpim",       icon:"check2"},
  {key:"kabag",         label:"Kabag Prokopim",                icon:"check"},
];
const WF={
  draft:             {label:"Draft",             color:"#64748b",bg:"#f1f5f9"},
  menunggu_kasubbag_protokol: {label:"Menunggu Kasubbag Protokol", color:"#d97706",bg:"#fef3c7"},
...
const PAKAIAN=["PDH","PDH Batik Tarakan","Batik Lengan Panjang","PSL","PSR","PSH","PDUB","Pakaian Muslim","Pakaian Lapangan","Pakaian Olahraga","Bebas Rapi","Lainnya"];
...
async function initUsers(){
  ...
  try{
    ...
    if(rows&&rows.length>0){
      ...
      // Migrasi username kasubbag_kominfo → kasubbag_komdokpim
      const hasOldKominfo=rows.some(u=>u.username==="kasubbag_kominfo"||u.role==="kasubbag_kominfo");
      if(hasOldKominfo){
        const renamed=rows.map(u=>{
          if(u.username==="kasubbag_kominfo"||u.role==="kasubbag_kominfo"){
            return {...u,
              username:u.username==="kasubbag_kominfo"?"kasubbag_komdokpim":u.username,
              role:"kasubbag_komdokpim"
            };
          }
          return u;
        });
        await Promise.all(renamed.filter(u=>u.username==="kasubbag_komdokpim").map(u=>dbUpsertUser(u))).catch(()=>{});
        try{await fetch(SUPA_URL+"/rest/v1/users?username=eq.kasubbag_kominfo",{method:"DELETE",headers:H()});}catch{}
        rows.splice(0,rows.length,...renamed);
      }
...
const seed=[
  ...
  mkEv({id:4,tanggal:TMR,jam:"08:00",namaAcara:"Apel Pagi Gabungan",penyelenggara:"Sekretariat Daerah",kontak:"Hendra 0815-1111-2222",buktiUndangan:"Memo No.5/2025",pakaian:"PDH",lokasi:"Halaman Kantor Wali Kota Tarakan",jenisKegiatan:"Menghadiri",jenisKegiatanLainnya:"",catatan:"",untukPimpinan:["walikota"],alur:"menunggu_kabag"}),
];
const emptyForm={tanggal:"",jam:"",namaAcara:"",penyelenggara:"",kontak:"",buktiUndangan:"",pakaian:"PDH",pakaianLainnya:"",jenisKegiatan:"Menghadiri",catatan:"",lokasi:"",untukPimpinan:["walikota"],undanganFile:null,undanganNama:"",personil:[],catatanPenugasan:""};
...
function SambutanBlock({ev,canUpload,canVerify,onUploadDocx,onRemove,onVerify}){
  ...
  if(ev.sambutanFile){
    ...
    return <>
      ...
      <div style={{background:"linear-gradient(135deg,#ECFDF5,#D1FAE5)",borderRadius:12,padding:13,border:"1.5px solid #6EE7B7"}}>
        ...
        {ev.sambutanVerified&&<div style={{display:"flex",alignItems:"center",gap:6,padding:"6px 12px",background:"#D1FAE5",borderRadius:8,marginBottom:8,fontSize:12,color:"#065F46",fontWeight:700}}>
          <span>✅</span> Terverifikasi oleh Kasubbag Komdokpim
        </div>}
        {!ev.sambutanVerified&&canVerify&&<button onClick={()=>onVerify&&onVerify()} style={{width:"100%",padding:"10px",borderRadius:10,border:"2px solid #0D6B4F",background:"#ECFDF5",color:"#0D6B4F",cursor:"pointer",fontWeight:800,fontSize:13,marginBottom:8}}>
          ✓ Verifikasi Sambutan Ini
        </button>}
...
function SummaryModal({events,onToggleHide,onClose}){
  ...
  const filtered=events.filter(e=>{
    ...
    return false;
  }).filter(e=>e.alur==="disetujui").sort((a,b)=>a.jam.localeCompare(b.jam));

  const pub=filtered.filter(e=>!e.tersembunyi);

  const rangeLabel=mode==="today"?"*"+getHari(todayStr())+", "+fmt(todayStr())+"*":
    mode==="tomorrow"?"*"+getHari(tomorrowStr())+", "+fmt(tomorrowStr())+"*":
    "*"+fmt(from)+" s.d. "+fmt(to)+"*";

  const shareText=["*AGENDA KEGIATAN PIMPINAN*",rangeLabel,"",...pub.flatMap((ev,i)=>[
    "*"+(i+1)+". "+ev.namaAcara+"*",
    "&#128197; "+getHari(ev.tanggal)+", "+fmt(ev.tanggal),
    "&#128336; "+ev.jam+" WITA",
    ev.penyelenggara?"Penyelenggara: "+ev.penyelenggara:"",
    ev.lokasi?"&#128205; "+ev.lokasi:"",
    ev.pakaian&&ev.pakaian!=="PDH"?"Pakaian: "+ev.pakaian:"",
    "",
  ].filter(Boolean)),"_Disiapkan oleh Protokol & Komunikasi Pimpinan_"].join("\n");
  ...
  const print=()=>{const w=window.open("","_blank");w.document.write("<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Agenda</title><style>body{font-family:Arial,sans-serif;padding:20px;white-space:pre-wrap;font-size:14px;line-height:1.8;}</style></head><body>"+shareText.replace(/\n/g,"<br>")+"</body></html>");w.document.close();w.focus();setTimeout(()=>w.print(),400);};
  ...
  return <div style={{position:"fixed",inset:0,zIndex:8100,background:"rgba(0,0,0,0.55)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
    <div style={{background:"white",borderRadius:16,width:"100%",maxWidth:500,maxHeight:"88vh",display:"flex",flexDirection:"column"}}>
      <div style={{padding:"16px 20px 12px",borderBottom:"1px solid #f1f5f9",display:"flex",alignItems:"center",gap:10}}>
        <div style={{flex:1}}><div style={{fontSize:16,fontWeight:700,color:NAVY}}>Rekap Agenda WA</div><div style={{fontSize:12,color:"#64748b"}}>{pub.length} kegiatan dipilih</div></div>
        <button onClick={onClose} style={{background:"#f1f5f9",border:"none",borderRadius:7,padding:"6px 10px",cursor:"pointer",fontSize:13,color:"#475569"}}>&#10005;</button>
      </div>

      {/* Mode selector */}
      <div style={{padding:"12px 20px 0",display:"flex",gap:8,flexWrap:"wrap"}}>
        {[{k:"today",l:"Hari Ini"},{k:"tomorrow",l:"Besok"},{k:"range",l:"Rentang Tertentu"}].map(({k,l})=>(
          <button key={k} onClick={()=>setMode(k)} style={{padding:"7px 14px",borderRadius:20,border:"1.5px solid "+(mode===k?NAVY:"#e2e8f0"),background:mode===k?NAVY:"white",color:mode===k?"white":"#475569",cursor:"pointer",fontSize:12,fontWeight:mode===k?700:400}}>
            {l}
          </button>
        ))}
      </div>
      ...
      {/* Range inputs */}
      {mode==="range"&&<div style={{padding:"10px 20px 0",display:"flex",gap:8,alignItems:"center"}}>
        <div style={{flex:1}}><label style={{fontSize:11,color:"#64748b",fontWeight:600,display:"block",marginBottom:2}}>Dari</label><input type="date" value={from} onChange={e=>setFrom(e.target.value)} style={{width:"100%",padding:"7px 10px",border:"1.5px solid #e2e8f0",borderRadius:8,fontSize:13}}/></div>
        <span style={{color:"#94a3b8",marginTop:14}}>s.d.</span>
        <div style={{flex:1}}><label style={{fontSize:11,color:"#64748b",fontWeight:600,display:"block",marginBottom:2}}>Sampai</label><input type="date" value={to} onChange={e=>setTo(e.target.value)} style={{width:"100%",padding:"7px 10px",border:"1.5px solid #e2e8f0",borderRadius:8,fontSize:13}}/></div>
      </div>}

      <div style={{flex:1,overflowY:"auto",padding:"14px 20px 20px"}}>
        {filtered.length===0?<div style={{textAlign:"center",padding:"30px",color:"#94a3b8",fontSize:14}}>Tidak ada agenda untuk periode ini</div>:
        <>{filtered.map(ev=><div key={ev.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,marginBottom:8,background:"#f8fafc",border:"1px solid #e2e8f0"}}>
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:11,color:"#64748b"}}>{mode==="range"?getHari(ev.tanggal)+", "+fmt(ev.tanggal)+" | ":""}{ev.jam} | {ev.penyelenggara}</div>
          </div>
          <button onClick={()=>onToggleHide(ev.id)} style={{flexShrink:0,padding:"5px 10px",borderRadius:8,border:"1.5px solid "+(ev.tersembunyi?"#e2e8f0":"#0A1628"),background:ev.tersembunyi?"#f1f5f9":"#0A1628",color:ev.tersembunyi?"#94a3b8":"white",cursor:"pointer",fontSize:11,fontWeight:700}}>
            {ev.tersembunyi?"+ Masukkan":"- Keluarkan"}
          </button>
        </div>)}
        {pub.length>0&&<>
          <div style={{background:"#f8fafc",borderRadius:10,padding:12,border:"1px solid #e2e8f0",marginTop:6,marginBottom:12}}><pre style={{margin:0,fontSize:12,whiteSpace:"pre-wrap",color:"#1e293b",fontFamily:"inherit"}}>{shareText}</pre></div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            <button onClick={copy} style={{padding:"12px",borderRadius:10,border:"none",background:NAVY,color:"white",cursor:"pointer",fontSize:13,fontWeight:700}}>&#128203; Salin Teks WA</button>
            <button onClick={print} style={{padding:"12px",borderRadius:10,border:"1.5px solid "+NAVY,background:"white",color:NAVY,cursor:"pointer",fontSize:13,fontWeight:700}}>&#128438; Cetak</button>
          </div>
        </>}</>}
      </div>
    </div>
      ...
      {pending.map(ev=>(
        <div key={ev.id} style={{background:"white",borderRadius:16,overflow:"hidden",boxShadow:"0 3px 14px rgba(0,0,0,0.08)",border:"2px solid "+(isKasubbag?"#fde68a":"#c4b5fd")}}>
          ...
          <div style={{padding:"14px 16px",borderBottom:"1px solid #f1f5f9"}}>
            ...
            {ev.submittedBy&&<div style={{fontSize:11,color:"#94a3b8",marginTop:6}}>Diajukan oleh: {getNamaByUsername(ev.submittedBy)}</div>}
          </div>
...
function FormView({form,setForm,editId,isMobile,onSubmit,onCancel,onOpenAI,onUndanganUpload,showT}){
  ...
  const JENIS_COLORS={Sambutan:{bg:"#fdf4ff",a:"#9333ea","Lainnya":{a:"#475569",bg:"#f1f5f9"}},Pengarahan:{bg:"#eff6ff",a:"#2563eb"},Menghadiri:{bg:"#f0fdf4",a:"#16a34a"}};
    ...
    <div style={{background:"white",borderRadius:12,padding:isMobile?"14px":"24px",
      ...
      {(wizStep===1||editId)&&<>
        ...
        <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:"0 20px"}}>
          ...
          <div style={{marginBottom:12}}>
            <label style={{display:"block",fontSize:12,color:"#475569",fontWeight:600,marginBottom:4}}>Jam *</label>
      ...
      {wizStep===2&&!editId&&<>
        ...
        <div style={{marginBottom:12}}>
          ...
          <div style={{display:"flex",gap:6}}>
            {["Sambutan","Pengarahan","Menghadiri","Lainnya"].map(j=>(
        ...
        <div style={{marginBottom:12}}>
          ...
          <select value={form.pakaian||"PDH"} onChange={e=>setForm(p=>({...p,pakaian:e.target.value}))}
            ...
            {(form.pakaian==="Lainnya")&&<input type="text" placeholder="Tulis jenis pakaian..." value={form.pakaianLainnya||""} onChange={e=>setForm(p=>({...p,pakaianLainnya:e.target.value}))} style={{width:"100%",padding:"8px 11px",border:"1.5px solid #e2e8f0",borderRadius:8,fontSize:13,marginTop:6}}/>}
...
async function sendWA(params){
  if(!params.to){console.warn("sendWA: no 'to' number, skip");return;}
  try{
    ...
    if(!res.ok){const d=await res.json().catch(()=>({}));console.warn("WA gagal:",d);}
...
// Helper: dapat nama dari username
function getNamaFromUsername(username){
  if(!username)return username||"";
  try{
    const users=loadUsers();
    const u=users.find(u=>u.username===username);
    return u?u.nama:username;
  }catch{return username;}
}
...
export default function App(){
  ...
  const doLogin=async()=>{
    ...
    setUser(candidate);setTab(candidate.role==="ajudan"?"ajudan":"jadwal");
  ...
  useEffect(()=>{
    ...
    boot();
    // Polling realtime setiap 15 detik
    const pollInterval=setInterval(async()=>{
      if(!SUPA_OK)return;
      try{
        const rows=await dbLoadAll();
        if(rows&&rows.length>0)setEvents(prev=>{
  ...
  // ── Realtime: poll Supabase setiap 10 detik untuk update otomatis ──
  useEffect(()=>{
  ...
  const saveEvaluasi=(evId, entry)=>{
    ...
    for(const un of personilArr){
      ...
      if(u){
        ...
        if(u.noWA){
          sendWA({to:u.noWA,namaAcara:ev?.namaAcara,tanggal:ev?.tanggal,jam:ev?.jam,penyelenggara:ev?.penyelenggara,lokasi:ev?.lokasi,event:"penugasan",namaPersonil:u.nama,catatanPenugasan});
        }
      }
    }
  };
  ...
  const submit=()=>{
    ...
    else{const n={...form,id:Date.now(),alur:"draft",catatanTolak:"",statusWK:null,statusWWK:null,perwakilanWK:"",perwakilanWWK:"",delegasiKeWWK:false,sambutanFile:null,sambutanNama:"",catatanPimpinan:"",tersembunyi:false,alurHapus:null,submittedBy:user?.username};setEvents(p=>[...p,n]);dbUpsert(n).catch(console.error);if(conflict)showT("Potensi tabrakan jadwal!","warn");else showT("Draft disimpan. Kirim ke Kasubbag Protokol.");}
  ...
  if(!user){
    const features=[
      ["📋","Workflow Approval","Staf → Kasubbag Protokol → Kabag"],
      ...
      {navGroups.map(group=><div key={group.label} style={{marginBottom:8}}>
        ...
        {group.items.map(item=>{const isActive=tab===item.key&&!item.key.startsWith("action:");return(
          <button key={item.key+item.label} className="nav-btn sidebar-link" onClick={()=>handleNavClick(item.key)} style={{width:"100%",padding:"9px 14px",borderRadius:9,border:"none",background:isActive?"rgba(255,255,255,0.18)":"transparent",color:"white",cursor:"pointer",fontSize:12.5,fontWeight:isActive?700:400,textAlign:"left",display:"flex",alignItems:"center",gap:10,marginBottom:2,borderLeft:isActive?"3px solid "+th.a:"3px solid transparent",opacity:isActive?1:0.72}}>
            <span style={{fontSize:15,minWidth:22,textAlign:"center"}}>{item.icon}</span>
            {item.label}
            {item.key==="jadwal"&&(KASUBBAG_ROLES.includes(role)||role==="kabag")&&(()=>{
...
// ═══════════════════════════════════════════════════════════════════════
function AjudanDashboard({events, upd, showT, setDelegTarget, isMobile}){
  const NAVY="#0A1628",GOLD="#C9A84C",GREEN="#0D6B4F";
  const now = new Date();
  ...
  const needsConfirm = approved.filter(e=>{
    const hasWK = e.untukPimpinan.includes("walikota") && !e.statusWK && !e.delegasiKeWWK;
    const hasWWK = (e.untukPimpinan.includes("wakilwalikota")||e.delegasiKeWWK) && !e.statusWWK;
...
function OnboardingModal({role, onClose}){
  ...
  const hints = {
    staf:[
      ...
      {icon:"📤",title:"Kirim ke Kasubbag Protokol",desc:"Setelah draft tersimpan, buka kartu → tombol 'Kirim ke Kasubbag Protokol'"},
    ...
    staf_input:[
      ...
      {icon:"📤",title:"Kirim ke Kasubbag Protokol",desc:"Buka kartu jadwal → tombol 'Kirim ke Kasubbag Protokol' untuk proses approval"},
    ...
    kasubbag_komdokpim:[
      {icon:"📝",title:"Upload Sambutan",desc:"Buka kartu jadwal Sambutan yang disetujui → bagian 'Naskah Sambutan' → upload file DOCX"},
      {icon:"✅",title:"Verifikasi Sambutan",desc:"Setelah sambutan diupload → klik 'Verifikasi Sambutan Ini' untuk mengesahkan naskah"},
      {icon:"👥",title:"Tugaskan Tim Dokumentasi",desc:"Buka kartu jadwal → tombol 'Tugaskan Personil' → pilih anggota Tim Komunikasi yang mendampingi"},
    ],
  };
...
// ==================== FORM ====================
  ...
  const EventCard=({ev})=>{
    ...
    return <div id={"ev-"+ev.id} className="ev-card" style={{background:cardBg,borderRadius:16,marginBottom:10,boxShadow:"0 2px 12px rgba(0,0,0,0.07),0 0 0 1px rgba(0,0,0,0.04)",border:"1.5px solid "+borderColor,overflow:"hidden",opacity:cardOpacity}}>
      ...
      {["kabag","ajudan","kasubbag_protokol","timkom","staf","staf_input"].includes(role)&&ev.alur==="disetujui"&&(()=>{
        const all=loadUsers();
    ...
    return <div>
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":ev.jenisKegiatan==="Sambutan"?"1fr 1fr":"1fr",gap:"0 24px",marginBottom:14}}>
        ...
        {ev.jenisKegiatan==="Sambutan"&&<div>
          <SambutanBlock ev={ev} canUpload={role==="timkom"||role==="kasubbag_komdokpim"} canVerify={role==="kasubbag_komdokpim"} onVerify={()=>{upd(ev.id,{sambutanVerified:true,sambutanVerifiedAt:new Date().toISOString()});showT("Sambutan terverifikasi ✓");}} onUploadDocx={(f)=>handleSambutanDocx(ev.id,f,ev)} onRemove={()=>{if(ev.sambutanFile&&!ev.sambutanFile.startsWith("data:"))storageDelete("sambutan",ev.sambutanFile).catch(()=>{});if(ev.sambutanDocx&&!ev.sambutanDocx.startsWith("data:")&&!ev.sambutanDocx.startsWith("blob:"))storageDelete("sambutan",ev.sambutanDocx).catch(()=>{});updAndSync(ev.id,{sambutanFile:null,sambutanNama:"",sambutanDocx:null,sambutanDocxNama:""});showT("Naskah sambutan dihapus","warn");}}/>
        </div>}
      </div>
      ...
      {role==="kasubbag_protokol"&&<div style={{display:"flex",flexDirection:"column",gap:7}}>
        {ev.alur==="menunggu_kasubbag"&&!ev.alurHapus&&<><button onClick={()=>{upd(ev.id,{alur:"menunggu_kabag"});showT("Diteruskan ke Kabag");
          ...
          <div style={{borderRadius:10,overflow:"hidden",border:"1.5px solid #fecaca"}}>
            ...
            <button onClick={()=>askConfirm("Tolak & Kembalikan ke Staf?","Jadwal '"+ev.namaAcara+"' akan dikembalikan dengan catatan penolakan.",()=>{upd(ev.id,{alur:"ditolak",catatanTolak:rejectTexts[ev.id]||""});showT("Dikembalikan","warn");const submitter=loadUsers().find(u=>u.username===ev.submittedBy);if(submitter?.noWA)sendWA({to:submitter.noWA,namaAcara:ev.namaAcara,tanggal:ev.tanggal,jam:ev.jam,penyelenggara:ev.penyelenggara,event:"rejected",catatanTolak:rejectTexts[ev.id]||""});sendPush({targetRole:"staf",title:"❌ Jadwal Dikembalikan",body:ev.namaAcara+": "+(rejectTexts[ev.id]||"Perlu diperbaiki"),url:"/",tag:"rejected-"+ev.id});sendPush({targetRole:"staf_input",title:"❌ Jadwal Dikembalikan",body:ev.namaAcara+": "+(rejectTexts[ev.id]||"Perlu diperbaiki"),url:"/",tag:"rejected-"+ev.id});},"Tolak","#991b1b")} style={{width:"100%",padding:"10px",border:"none",background:"#fee2e2",color:"#991b1b",cursor:"pointer",fontSize:12,fontWeight:700}}>Tolak & Kembalikan ke Staf</button>
          </div></>}
      ...
      {role==="kabag"&&<div style={{display:"flex",flexDirection:"column",gap:7}}>
        {ev.alur==="menunggu_kabag"&&!ev.alurHapus&&<><button onClick={()=>{upd(ev.id,{alur:"disetujui"});showT("Jadwal disetujui & dipublikasi");
            const submitter=loadUsers().find(u=>u.username===ev.submittedBy);if(submitter?.noWA)sendWA({to:submitter.noWA,namaAcara:ev.namaAcara,tanggal:ev.tanggal,jam:ev.jam,penyelenggara:ev.penyelenggara,lokasi:ev.lokasi,event:"approved"});sendPush({targetRole:"staf",title:"✅ Jadwal Disetujui",body:ev.namaAcara+" sudah dipublikasi",url:"/",tag:"approved-"+ev.id});sendPush({targetRole:"staf_input",title:"✅ Jadwal Disetujui",body:ev.namaAcara+" sudah dipublikasi",url:"/",tag:"approved-"+ev.id});;}} style={{padding:"11px",borderRadius:10,border:"none",background:NAVY,color:"white",cursor:"pointer",fontSize:13,fontWeight:700}}>Setujui & Publikasi</button>
          <div style={{borderRadius:10,overflow:"hidden",border:"1.5px solid #fecaca"}}>
            <textarea placeholder="Catatan penolakan..." value={rejectTexts[ev.id]||""} onChange={e=>setRT(p=>({...p,[ev.id]:e.target.value}))} rows={2} style={{width:"100%",padding:"9px 11px",border:"none",resize:"none",color:"#334155",background:"white",fontSize:13}}/>
            <button onClick={()=>askConfirm("Tolak Jadwal?","Jadwal '"+ev.namaAcara+"' akan dikembalikan ke staf.",()=>{upd(ev.id,{alur:"ditolak",catatanTolak:rejectTexts[ev.id]||""});showT("Ditolak","warn");const submitter=loadUsers().find(u=>u.username===ev.submittedBy);if(submitter?.noWA)sendWA({to:submitter.noWA,namaAcara:ev.namaAcara,tanggal:ev.tanggal,jam:ev.jam,penyelenggara:ev.penyelenggara,event:"rejected",catatanTolak:rejectTexts[ev.id]||""});sendPush({targetRole:"staf",title:"❌ Jadwal Dikembalikan",body:ev.namaAcara+": "+(rejectTexts[ev.id]||"Perlu diperbaiki"),url:"/",tag:"rejected-"+ev.id});sendPush({targetRole:"staf_input",title:"❌ Jadwal Dikembalikan",body:ev.namaAcara+": "+(rejectTexts[ev.id]||"Perlu diperbaiki"),url:"/",tag:"rejected-"+ev.id});},"Tolak","#991b1b")} style={{width:"100%",padding:"10px",border:"none",background:"#fee2e2",color:"#991b1b",cursor:"pointer",fontSize:12,fontWeight:700}}>Tolak & Kembalikan</button>
          </div></>}
        {ev.alurHapus==="menunggu_kabag"&&<><div style={{background:"#fff1f2",borderRadius:9,padding:"9px 12px",fontSize:13,color:"#e11d48"}}>Permintaan penghapusan (sudah disetujui Kasubbag Protokol)</div>
