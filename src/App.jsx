      ...
      {items.map(v=>(
        <div key={v} onClick={()=>onPick(v)}
          style={{height:44,display:"flex",alignItems:"center",justifyContent:"center",
            ...
            background:v===sel?"#EBF0FA":"transparent",borderRadius:v===sel?8:0,
...
function FormView({form,setForm,editId,setEditId,setTab,isMobile,onSubmit,onCancel,onOpenAI,onUndanganUpload,showT}){
  return <div style={{background:"white",borderRadius:12,padding:isMobile?"16px":"24px",boxShadow:"0 2px 12px rgba(0,0,0,0.07)",border:"1.5px solid "+GOLD,animation:"up 0.2s ease"}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
      ...
      {!editId&&<button onClick={onOpenAI} style={{padding:"8px 14px",borderRadius:9,border:"none",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"white",cursor:"pointer",fontSize:12,fontWeight:700,display:"flex",alignItems:"center",gap:6,boxShadow:"0 4px 14px rgba(99,102,241,0.35)"}}><span style={{fontSize:16}}>🤖</span>{isMobile?"AI Auto-Isi":"AI Analisa Undangan"}</button>}
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:"0 20px"}}>
      {[{type:"date",k:"tanggal",l:"Tanggal *"},{type:"text",k:"namaAcara",l:"Nama Acara *",full:true},{type:"text",k:"penyelenggara",l:"Penyelenggara"},{type:"text",k:"kontak",l:"Kontak"},{type:"text",k:"buktiUndangan",l:"No. Surat / Bukti Undangan"}].map(f=>(
    ...
    <div style={{display:"flex",gap:10}}>
      <button onClick={onCancel} style={{flex:1,padding:"12px",borderRadius:10,border:"1.5px solid #e2e8f0",background:"white",cursor:"pointer",fontSize:13,fontWeight:600,color:"#64748b"}}>Batal</button>
      <button onClick={onSubmit} className="btn-primary" style={{flex:2,padding:"12px",borderRadius:10,border:"none",background:NAVY,color:"white",cursor:"pointer",fontSize:13,fontWeight:700}}>Simpan sebagai Draft</button>
    </div>
  </div>;
}
...
export default function App(){
  ...
  const CSS=`
    ...
    *{box-sizing:border-box;-webkit-font-smoothing:antialiased;font-family:'Plus Jakarta Sans',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;}
    ...
    .time-col{-ms-overflow-style:none;scrollbar-width:none;}
  `;
  ...
  return <div style={{minHeight:"100vh",width:"100%",background:NAVY,display:"flex"}}>
    ...
    {isMobile?<div style={{width:"100%",minHeight:"100vh",display:"flex",flexDirection:"column",background:"#F2F4F8"}}><MobileHeader/><MainContent/></div>:<div style={{width:"100%",minHeight:"100vh",display:"flex",background:NAVY}}><Sidebar/><MainContent/></div>}
  </div>;
}
