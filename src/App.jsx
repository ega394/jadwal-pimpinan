...
const ROLES_WITH_REPORT=[“staf”,“staf_input”,“kasubbag”,“kabag”,“timkom”];
...
const mkEv=o=>({alur:“disetujui”,catatanTolak:””,statusWK:null,statusWWK:null,perwakilanWK:””,perwakilanWWK:””,delegasiKeWWK:false,sambutanFile:null,sambutanNama:””,undanganFile:null,undanganNama:””,catatanPimpinan:””,tersembunyi:false,alurHapus:null,lokasi:””,personil:[],catatanPenugasan:””,evaluasi:{},…o});
const seed=[
...
const shareText=[”*AGENDA KEGIATAN PIMPINAN*”,”*”+getHari(today)+”, “+fmt(today)+”*”,””,…pub.flatMap((ev,i)=>[`*${i+1}. ${ev.namaAcara}*`,ev.jam+” WITA”,ev.penyelenggara,…(ev.lokasi?[“Lokasi: “+ev.lokasi]:[]),“Pakaian: “+ev.pakaian,…(ev.catatan?[“Catatan: “+ev.catatan]:[]),””]),”*Bagian Protokol & Komunikasi Pimpinan*”].join(”\n”);
...
const isKominfo=currentUser.role===“timkom”;
...
const ELIGIBLE_ROLES=[“staf”,“staf_input”,“kasubbag”,“timkom”];
const candidates=allUsers.filter(u=>ELIGIBLE_ROLES.includes(u.role));
...
React.useEffect(()=>{
if(role!==“kasubbag”&&role!==“timkom”)return;
const now=new Date();
...
const TH={walikota:{g:“linear-gradient(135deg,”+NAVY+”,#1B4080)”,a:GOLD},wakilwalikota:{g:“linear-gradient(135deg,#053f2a,#065f46)”,a:”#6ee7b7”},ajudan:{g:“linear-gradient(135deg,#1e293b,#334155)”,a:”#94a3b8”},timkom:{g:“linear-gradient(135deg,#3730a3,#4f46e5)”,a:”#a5b4fc”},staf:{g:“linear-gradient(135deg,”+NAVY+”,#1B4080)”,a:GOLD},staf_input:{g:“linear-gradient(135deg,#1e3a5f,#2563eb)”,a:”#93c5fd”},kasubbag:{g:“linear-gradient(135deg,#78350f,#d97706)”,a:”#fde68a”},kabag:{g:“linear-gradient(135deg,#064e3b,#10b981)”,a:”#6ee7b7”}};
...
<style>{CSS+`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap'); @keyframes fadeUp{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}} @keyframes slideRight{from{opacity:0;transform:translateX(30px)}to{opacity:1;transform:translateX(0)}} @keyframes pulseGlow{0%,100%{box-shadow:0 0 40px -10px hsla(42,78%,55%,0.2)}50%{box-shadow:0 0 40px -10px hsla(42,78%,55%,0.45)}} @keyframes focusBorder{to{border-color:hsl(42,78%,55%)}} .login-inp:focus{border-color:hsl(42,78%,55%)!important;box-shadow:0 0 0 3px hsla(42,78%,55%,0.15)!important;} .login-btn-primary{background:hsl(42,78%,55%);color:hsl(215,30%,10%);border:none;font-weight:800;font-family:'Plus Jakarta Sans',sans-serif;cursor:pointer;transition:all 0.2s;} .login-btn-primary:hover{background:hsl(42,85%,70%);box-shadow:0 4px 20px hsla(42,78%,55%,0.4);transform:translateY(-1px);} .login-btn-primary:active{transform:scale(0.97);} .login-btn-outline{background:transparent;border:1.5px solid hsla(215,20%,60%,0.25);color:${MUTED};font-family:'Plus Jakarta Sans',sans-serif;cursor:pointer;transition:all 0.2s;} .login-btn-outline:hover{border-color:hsla(42,78%,55%,0.4);color:${FG};} .feat-item{animation:fadeUp 0.6s ease both;} .login-card{animation:slideRight 0.6s ease both;}`}</style>
...
…((role===“staf”||role===“staf_input”)?[{key:“jadwal”,icon:“📅”,label:“Jadwal Saya”},{key:“form”,icon:“✏️”,label:“Input Jadwal Baru”}]:[]),
...
const mobTabs=[
…((role===“staf”||role===“staf_input”)?[{key:“jadwal”,label:“Jadwal”,icon:“📅”},{key:“form”,label:“Input”,icon:“✏️”}]:[]),
...
```
  ...
  {/* TIMKOM PENUGASAN */}
  {role==="timkom"&&ev.alur==="disetujui"&&<div style={{marginBottom:8}}>
    <button onClick={()=>setPenugasanEv(ev)} style={{width:"100%",padding:"10px",borderRadius:10,border:"1.5px dashed #a5b4fc",background:"#eef2ff",color:"#4338ca",cursor:"pointer",fontSize:12,fontWeight:700}}>
      {(!ev.personil||ev.personil.length===0)?"👥 Tugaskan Personil":"👥 Edit Penugasan ("+(ev.personil?.length||0)+" personil)"}
    </button>
  </div>}
...
// ==================== MAIN CONTENT ====================
const pageTitle=tab===“tayang”?“Agenda Kegiatan Pimpinan”:tab===“form”?“Input Jadwal Baru”:tab===“semua”?“Semua Jadwal”:tab===“jadwal”?role===“kasubbag”||role===“kabag”?“Antrian Approval”:role===“staf”||role===“staf_input”?“Jadwal & Penugasan Saya”:“Jadwal Saya”:“Jadwal”;
...
{(role==="kasubbag"||role==="timkom")&&notifPenugasan.length>0&&tab!=="form"&&
