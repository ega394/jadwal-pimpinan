...
function makeICS(ev){const[y,mo,d]=ev.tanggal.split("-");const[hh,mm]=ev.jam.split(":");const p=n=>String(n).padStart(2,"0");const ds=y+mo+d+"T"+p(hh)+p(mm)+"00";const de=y+mo+d+"T"+p(parseInt(hh)+2)+p(mm)+"00";return"data:text/calendar;charset=utf8,"+encodeURIComponent(["BEGIN:VCALENDAR","VERSION:2.0","BEGIN:VEVENT","UID:ev"+ev.id+"@protokol","DTSTART:"+ds,"DTEND:"+de,"SUMMARY:"+ev.namaAcara,"DESCRIPTION:"+ev.penyelenggara+"\\nPakaian: "+ev.pakaian,...(ev.lokasi?["LOCATION:"+ev.lokasi]:[]),"END:VEVENT","END:VCALENDAR"].join("\r\n"));}
...
function UndanganBlock({ev,canEdit,onUpload,onRemove}){
  ...
  if(ev.undanganFile)return <>
    {view&&<FileViewModal file={ev.undanganFile} nama={ev.undanganNama} onClose={()=>setV(false)}/>}
    <div style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:"#f0f9ff",borderRadius:8,border:"1px solid #bae6fd"}}>
      <span style={{fontSize:15}}>{ev.undanganNama?.match(/\.(jpg|jpeg|png)$/i)?"image":"pdf"==="pdf"?"pdf":"img"}</span>
      <span style={{flex:1,fontSize:12,fontWeight:600,color:"#0c4a6e",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ev.undanganNama||"Berkas Undangan"}</span>
      <button onClick={()=>setV(true)} style={{padding:"4px 9px",borderRadius:6,border:"1px solid #0284c7",background:"white",color:"#0284c7",cursor:"pointer",fontSize:11,fontWeight:700,flexShrink:0}}>Lihat</button>
      <a href={ev.undanganFile} download={ev.undanganNama} style={{padding:"4px 9px",borderRadius:6,border:"none",background:"#0284c7",color:"white",cursor:"pointer",fontSize:11,fontWeight:700,textDecoration:"none",flexShrink:0}}>Unduh</a>
      {canEdit&&<button onClick={onRemove} style={{padding:"4px 8px",borderRadius:6,border:"1px solid #fca5a5",background:"white",color:"#ef4444",cursor:"pointer",fontSize:11,fontWeight:700,flexShrink:0}}>Hapus</button>}
    </div>
  </>;
  if(!canEdit)return <span style={{fontSize:12,color:"#94a3b8",fontStyle:"italic"}}>Tidak ada berkas</span>;
  return <>
    ...
    <button onClick={()=>ref.current.click()} disabled={load} style={{padding:"7px 14px",borderRadius:8,border:"1.5px dashed #94a3b8",background:"white",color:"#64748b",cursor:load?"default":"pointer",fontSize:12,fontWeight:600,display:"inline-flex",alignItems:"center",gap:6}}>
      {load?"Mengunggah...":"Unggah Berkas (PDF/JPG) - Opsional"}
    </button>
  </>;
}
...
export default function App(){
  ...
  const[delegTarget,setDelegTarget]=useState(null);const[expandedId,setExp]=useState(null);const[rejectTexts,setRT]=useState({});const[catatanInput,setCatatanInput]=useState({});
  ...
  // ==================== LOGIN ====================
  if(!user)return(
    <div style={{minHeight:"100vh",width:"100%",background:"linear-gradient(160deg,"+NAVY+" 0%,#1B4080 55%,#0d3d2e 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <style>{CSS}</style>
      <div style={{width:"100%",maxWidth:960,display:"flex",gap:0,borderRadius:20,overflow:"hidden",boxShadow:"0 32px 80px rgba(0,0,0,0.45)"}}>
        {/* Left panel */}
        <div style={{flex:1,background:"rgba(255,255,255,0.06)",backdropFilter:"blur(20px)",padding:"48px 40px",display:"flex",flexDirection:"column",justifyContent:"space-between",borderRight:"1px solid rgba(255,255,255,0.08)"}}>
          <div>
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:32}}>
              <img src="/logo_tarakan.png" alt="Logo" style={{height:64,width:"auto",objectFit:"contain",filter:"drop-shadow(0 2px 8px rgba(0,0,0,0.4))",flexShrink:0}} onError={e=>{e.target.style.display="none";e.target.nextSibling.style.display="flex";}}/>
              <div style={{width:60,height:60,borderRadius:"50%",background:"rgba(201,168,76,0.15)",border:"2px solid "+GOLD,display:"none",alignItems:"center",justifyContent:"center",fontSize:28,flexShrink:0}}>Logo</div>
              <div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {[["Workflow Approval","Staf - Kasubbag - Kabag"],["Analisa AI","Upload undangan, isi otomatis"],["Laporan PDF","Cetak rekap A4 landscape"],["Multi Platform","Desktop, tablet & mobile"]].map(([t,d])=><div key={t} style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:7,height:7,borderRadius:"50%",background:GOLD,flexShrink:0}}/><div><span style={{color:"white",fontSize:13,fontWeight:600}}>{t}</span><span style={{color:"rgba(255,255,255,0.45)",fontSize:12}}> — {d}</span></div></div>)}
            </div>
          </div>
        ...
        <div style={{width:360,background:"white",padding:"40px 36px",display:"flex",flexDirection:"column",justifyContent:"center"}}>
          ...
          <div style={{marginBottom:6}}>
            ...
            <div style={{position:"relative"}}>
              ...
              <button onClick={()=>setShowPass(p=>!p)} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#94a3b8",fontSize:15,padding:4}}>
                {showPass?"👁️":"👁️‍🗨️"}
              </button>
            </div>
          </div>
  ...
  const Sidebar=()=><aside style={{width:260,minHeight:"100vh",background:th.g,display:"flex",flexDirection:"column",flexShrink:0,position:"sticky",top:0,height:"100vh",overflowY:"auto",boxShadow:"4px 0 20px rgba(0,0,0,0.18)"}}>
  ...
  // ==================== MOBILE HEADER ====================
  const MobileHeader=()=><div style={{background:th.g,position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 14px rgba(0,0,0,0.2)"}}>
    <div style={{padding:"10px 12px",display:"flex",alignItems:"center",gap:7}}>
      <img src="/logo_tarakan.png" alt="" style={{height:28,width:"auto",objectFit:"contain",flexShrink:0}} onError={e=>e.target.style.display="none"}/>
      <div style={{flex:1,minWidth:0}}><div style={{color:"white",fontSize:12,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{user?.nama}</div><div style={{color:"rgba(255,255,255,0.5)",fontSize:9}}>{roleInfo.label}</div></div>
      {pendingList.length>0&&<button onClick={goToPending} style={{background:"#ef4444",color:"white",borderRadius:14,padding:"2px 8px",fontSize:10,fontWeight:700,border:"none",cursor:"pointer",flexShrink:0}}>{pendingList.length}</button>}
      <button onClick={()=>setShowSummary(true)} style={{background:"rgba(255,255,255,0.1)",border:"none",borderRadius:6,color:"white",padding:"5px 7px",cursor:"pointer",fontSize:11,fontWeight:700,flexShrink:0}}>WA</button>
      <button onClick={()=>setShowReport(true)} style={{background:"rgba(255,255,255,0.1)",border:"none",borderRadius:6,color:"white",padding:"5px 7px",cursor:"pointer",fontSize:11,fontWeight:700,flexShrink:0}}>PDF</button>
      {canReport&&<button onClick={()=>setShowLaporan(true)} style={{background:"rgba(255,255,255,0.1)",border:"none",borderRadius:6,color:"white",padding:"5px 7px",cursor:"pointer",fontSize:11,fontWeight:700,flexShrink:0}}>Lap</button>}
      {role==="kabag"&&<button onClick={()=>setShowAdmin(true)} style={{background:"rgba(255,255,255,0.1)",border:"none",borderRadius:6,color:th.a,padding:"5px 7px",cursor:"pointer",fontSize:11,fontWeight:700,flexShrink:0}}>Adm</button>}
      <button onClick={()=>setShowProfile(true)} style={{background:"rgba(255,255,255,0.1)",border:"none",borderRadius:6,color:"white",padding:"5px 7px",cursor:"pointer",fontSize:11,fontWeight:700,flexShrink:0}}>Akun</button>
      <div title={SUPA_OK?"DB OK":"Lokal"} style={{width:7,height:7,borderRadius:"50%",background:SUPA_OK?"#34d399":"#f87171",flexShrink:0}}/>
    </div>
    <div style={{display:"flex",borderTop:"1px solid rgba(255,255,255,0.1)",overflowX:"auto",background:"rgba(0,0,0,0.1)"}}>
      {[...( role==="staf"?[{key:"jadwal",label:"Jadwal"},{key:"form",label:"Input"}]:role==="kasubbag"||role==="kabag"?[{key:"jadwal",label:"Antrian"},{key:"semua",label:"Semua"}]:role==="walikota"||role==="wakilwalikota"||role==="ajudan"||role==="timkom"?[{key:"jadwal",label:"Jadwal"}]:[]),{key:"tayang",label:"Tayang"}].map(t=>(
        <button key={t.key} onClick={()=>{setTab(t.key);if(t.key==="form"){setForm(emptyForm);setEditId(null);}}} style={{padding:"8px 14px",border:"none",cursor:"pointer",background:"transparent",color:"white",fontSize:11,fontWeight:700,borderBottom:tab===t.key?"2.5px solid "+th.a:"2.5px solid transparent",opacity:tab===t.key?1:0.5,whiteSpace:"nowrap",flexShrink:0}}>{t.label}</button>
      ))}
    </div>
  </div>;

  // ==================== FORM ====================
  const FormView=()=><div style={{background:"white",borderRadius:12,padding:isMobile?"16px":"24px",boxShadow:"0 2px 12px rgba(0,0,0,0.07)",border:"1.5px solid "+GOLD,animation:"up 0.2s ease"}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
      <h2 style={{margin:0,color:NAVY,fontSize:isMobile?15:18,fontWeight:800}}>{editId?"Edit Jadwal":"Input Jadwal Baru"}</h2>
      {!editId&&<button onClick={()=>setShowAI(true)} style={{padding:"8px 14px",borderRadius:9,border:"none",background:"linear-gradient(135deg,#6366f1,#8b5cf6)",color:"white",cursor:"pointer",fontSize:12,fontWeight:700,display:"flex",alignItems:"center",gap:6}}>AI Analisa Undangan</button>}
    </div>
    <div style={{background:"#f0f9ff",borderRadius:8,padding:"9px 12px",marginBottom:14,border:"1px solid #bae6fd",fontSize:12,color:"#0284c7",fontWeight:600}}>Alur: Staf - Kasubbag Protokol - Kabag - Tayang</div>
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:"0 20px"}}>
      {[{type:"date",k:"tanggal",l:"Tanggal *"},{type:"time",k:"jam",l:"Jam *"},{type:"text",k:"namaAcara",l:"Nama Acara *",full:true},{type:"text",k:"penyelenggara",l:"Penyelenggara"},{type:"text",k:"kontak",l:"Kontak"},{type:"text",k:"buktiUndangan",l:"No. Surat / Bukti Undangan"}].map(f=>(
        <div key={f.k} style={{marginBottom:12,gridColumn:f.full?"1 / -1":"auto"}}>
          <label style={{display:"block",fontSize:12,color:"#475569",fontWeight:600,marginBottom:4}}>{f.l}</label>
          <input type={f.type} value={form[f.k]||""} onChange={e=>setForm(p=>({...p,[f.k]:e.target.value}))} style={{width:"100%",padding:"10px 12px",borderRadius:9,border:"1.5px solid #e2e8f0",color:"#1e293b",background:"white",fontSize:14}}/>
          {f.k==="tanggal"&&form.tanggal&&<div style={{marginTop:4,fontSize:12,color:NAVY,fontWeight:700}}>{getHari(form.tanggal)}, {fmt(form.tanggal)}</div>}
        </div>
      ))}
    </div>
    <div style={{marginBottom:12}}>
      <label style={{display:"block",fontSize:12,color:"#475569",fontWeight:600,marginBottom:4}}>Berkas Undangan (Opsional)</label>
      <UndanganBlock ev={form} canEdit={true} onUpload={async(file,name)=>{const b64=await new Promise((res,rej)=>{const r=new FileReader();r.onload=e=>res(e.target.result);r.onerror=rej;r.readAsDataURL(file);});setForm(p=>({...p,undanganFile:b64,undanganNama:name}));showT("Berkas siap diattach");}} onRemove={()=>setForm(p=>({...p,undanganFile:null,undanganNama:""}))}/>
    </div>
    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:"0 20px"}}>
      <div style={{marginBottom:12}}>
        <label style={{display:"block",fontSize:12,color:"#475569",fontWeight:600,marginBottom:4}}>Jenis Kegiatan</label>
        <div style={{display:"flex",gap:6}}>
          {JENIS.map(j=>{const jc={Sambutan:{bg:"#fdf4ff",a:"#9333ea"},Pengarahan:{bg:"#eff6ff",a:"#2563eb"},Menghadiri:{bg:"#f0fdf4",a:"#16a34a"}}[j];return <button key={j} onClick={()=>setForm(p=>({...p,jenisKegiatan:j}))} style={{flex:1,padding:"8px 4px",borderRadius:8,border:"1.5px solid "+(form.jenisKegiatan===j?jc.a:"#e2e8f0"),background:form.jenisKegiatan===j?jc.bg:"white",color:form.jenisKegiatan===j?jc.a:"#64748b",cursor:"pointer",fontSize:11,fontWeight:700}}>{j}</button>;})}
        </div>
      </div>
      <div style={{marginBottom:12}}>
        <label style={{display:"block",fontSize:12,color:"#475569",fontWeight:600,marginBottom:4}}>Pakaian</label>
        <select value={form.pakaian} onChange={e=>setForm(p=>({...p,pakaian:e.target.value}))} style={{width:"100%",padding:"10px 12px",borderRadius:9,border:"1.5px solid #e2e8f0",background:"white",color:"#1e293b",fontSize:14,WebkitAppearance:"none"}}>{PAKAIAN.map(x=><option key={x}>{x}</option>)}</select>
      </div>
    </div>
    <div style={{marginBottom:12}}>
      <label style={{display:"block",fontSize:12,color:"#475569",fontWeight:600,marginBottom:4}}>Catatan Penting</label>
      <textarea value={form.catatan} onChange={e=>setForm(p=>({...p,catatan:e.target.value}))} rows={2} style={{width:"100%",padding:"10px 12px",borderRadius:9,border:"1.5px solid #e2e8f0",color:"#1e293b",resize:"vertical",background:"white",fontSize:14}}/>
    </div>
    <div style={{marginBottom:16}}>
      <label style={{display:"block",fontSize:12,color:"#475569",fontWeight:600,marginBottom:4}}>Lokasi Acara</label>
      <div style={{display:"flex",gap:8}}>
        <input type="text" value={form.lokasi||""} onChange={e=>setForm(p=>({...p,lokasi:e.target.value}))} placeholder="Nama tempat / alamat" style={{flex:1,padding:"10px 12px",borderRadius:9,border:"1.5px solid #e2e8f0",color:"#1e293b",background:"white",fontSize:14}}/>
        {form.lokasi&&<a href={"https://www.google.com/maps/search/?api=1&query="+encodeURIComponent(form.lokasi)} target="_blank" rel="noopener noreferrer" style={{padding:"10px 14px",borderRadius:9,background:"#1a73e8",color:"white",textDecoration:"none",fontSize:13,fontWeight:700,display:"flex",alignItems:"center",flexShrink:0}}>Maps</a>}
      </div>
    </div>
    <div style={{marginBottom:18}}>
      ...
      <div style={{display:"flex",gap:10}}>
        {[{key:"walikota",label:"Wali Kota"},{key:"wakilwalikota",label:"Wakil Wali Kota"}].map(p=>(
          <label key={p.key} style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"10px",borderRadius:9,cursor:"pointer",border:form.untukPimpinan.includes(p.key)?"2px solid "+NAVY:"2px solid #e2e8f0",background:form.untukPimpinan.includes(p.key)?"#EBF0FA":"white",fontSize:12,fontWeight:700,color:form.untukPimpinan.includes(p.key)?NAVY:"#94a3b8"}}>
    ...
    <div style={{display:"flex",gap:10}}>
      <button onClick={()=>{setForm(emptyForm);setEditId(null);setTab("jadwal");}} style={{flex:1,padding:"12px",borderRadius:10,border:"1.5px solid #e2e8f0",background:"white",cursor:"pointer",fontSize:13,fontWeight:600,color:"#64748b"}}>Batal</button>
      <button onClick={submit} className="btn-primary" style={{flex:2,padding:"12px",borderRadius:10,border:"none",background:NAVY,color:"white",cursor:"pointer",fontSize:13,fontWeight:700}}>Simpan sebagai Draft</button>
    </div>
  </div>;
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":ev.jenisKegiatan==="Sambutan"?"1fr 1fr":"1fr",gap:"0 24px",marginBottom:14}}>
        <div>
          <div style={{display:"flex",flexDirection:"column",gap:5}}>
            ...
            {ev.undanganFile&&<div style={{display:"flex",gap:8,padding:"6px 10px",background:"#f8fafc",borderRadius:8,border:"1px solid #e2e8f0",alignItems:"center"}}>
              <div style={{minWidth:80,fontSize:10,color:"#94a3b8",fontWeight:700,textTransform:"uppercase"}}>Undangan</div>
              <div style={{flex:1}}><UndanganBlock ev={ev} canEdit={role==="staf"&&ev.alur!=="disetujui"} onUpload={(file,name)=>handleUndanganUpload(ev.id,file,name).then(()=>showT("Berkas undangan diupload"))} onRemove={()=>{if(ev.undanganFile&&!ev.undanganFile.startsWith("data:"))storageDelete("undangan",ev.undanganFile).catch(()=>{});updAndSync(ev.id,{undanganFile:null,undanganNama:""});}}/></div>
            </div>}
          </div>
          <a href={makeICS(ev)} download={ev.namaAcara+".ics"} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6,padding:"8px",borderRadius:9,border:"1.5px solid #e2e8f0",background:"white",color:"#334155",textDecoration:"none",fontSize:12,fontWeight:700,marginTop:8}}>Tambah ke Kalender</a>
        </div>
  ...
  const MainContent=()=><div style={{flex:1,minWidth:0,display:"flex",flexDirection:"column",background:"#F2F4F8",overflow:"hidden"}}>
    {/* Top bar */}
    ...
    <div style={{flex:1,overflowY:"auto",padding:isMobile?"10px 12px 100px":"24px 32px 48px"}}>
      ...
      {showForm?<FormView/>:listEvents.length===0?<div style={{textAlign:"center",padding:"60px 20px",color:"#94a3b8",background:"white",borderRadius:12,boxShadow:"0 1px 6px rgba(0,0,0,0.07)"}}><div style={{fontSize:48,marginBottom:14}}>Inbox</div><div style={{fontSize:16,fontWeight:600}}>Tidak ada jadwal</div><div style={{fontSize:13,marginTop:4}}>Belum ada data untuk filter yang dipilih</div></div>:
  ...
  return <div style={{minHeight:"100vh",width:"100%",background:NAVY,display:"flex"}}>
    ...
    {isMobile?<div style={{width:"100%",minHeight:"100vh",display:"flex",flexDirection:"column",background:"#F0F2F5"}}><MobileHeader/><MainContent/></div>:<div style={{width:"100%",minHeight:"100vh",display:"flex",background:NAVY}}><Sidebar/><MainContent/></div>}
  </div>;
}
