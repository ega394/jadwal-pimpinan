...
const path=evId+”/”+Date.now()+”*”+file.name.replace(/[^a-zA-Z0-9.*-]/g,”_”);
...
const[sel,setSel]=useState(””);const[cust,setCust]=useState(””);const fin=sel===”**c**”?cust:sel;
...
<button onClick={()=>setSel(”**c**”)} style={{padding:"10px 13px”,borderRadius:9,border:"1.5px solid "+(sel===”**c**”?NAVY:”#e2e8f0”),background:sel===”**c**”?”#EBF0FA”:"white”,color:sel===”**c**”?NAVY:”#334155”,cursor:"pointer”,fontSize:13,textAlign:"left”}}>Pejabat lainnya…</button>
</div>
{sel===”**c**”&&<input placeholder="Nama pejabat…” value={cust} onChange={e=>setCust(e.target.value)} style={{width:"100%”,padding:"10px 12px”,borderRadius:9,border:"1.5px solid #e2e8f0”,fontSize:14,marginBottom:12}}/>}
...
const shareText=[”*AGENDA KEGIATAN PIMPINAN*”,”*”+getHari(today)+”, "+fmt(today)+”*”,””,…pub.flatMap((ev,i)=>[`*${i+1}. ${ev.namaAcara}*`,ev.jam+” WIB”,ev.penyelenggara,…(ev.lokasi?["Lokasi: "+ev.lokasi]:[]),"Pakaian: "+ev.pakaian,…(ev.catatan?["Catatan: "+ev.catatan]:[]),””]),”*Bagian Protokol & Komunikasi Pimpinan*”].join(”\n”);
...
Isi PDF: kop surat • lokasi acara • kolom catatan kepala daerah • TTD Kabag Prokopim
</div>
...
const CSS=`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap'); html,body{margin:0;padding:0;width:100%;overflow-x:hidden;-webkit-text-size-adjust:100%;background:${NAVY};} *{box-sizing:border-box;-webkit-font-smoothing:antialiased;font-family:'Plus Jakarta Sans',-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;} input,select,textarea{font-family:inherit;} input[type=text],input[type=password],input[type=date],input[type=time],input[type=email],select,textarea{font-size:14px!important;-webkit-appearance:none;} a{-webkit-tap-highlight-color:transparent;color:inherit;} ::-webkit-scrollbar{width:5px;height:5px;} ::-webkit-scrollbar-track{background:transparent;} ::-webkit-scrollbar-thumb{background:#cbd5e1;border-radius:4px;} ::-webkit-scrollbar-thumb:hover{background:#94a3b8;} @keyframes up{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}} @keyframes slideIn{from{opacity:0;transform:translateX(-10px)}to{opacity:1;transform:translateX(0)}} @keyframes slideDown{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:translateY(0)}} @keyframes fadeIn{from{opacity:0}to{opacity:1}} @keyframes spin{to{transform:rotate(360deg)}} .nav-btn{transition:all 0.15s ease;border-radius:8px!important;} .nav-btn:hover{background:rgba(255,255,255,0.14)!important;opacity:1!important;transform:translateX(2px);} .card-row{transition:background 0.1s;} .card-row:hover{background:#f5f7ff!important;} .btn-primary{transition:all 0.15s ease;} .btn-primary:hover{opacity:0.88;transform:translateY(-1px);box-shadow:0 4px 16px rgba(11,37,69,0.25);} .btn-primary:active{transform:scale(0.98);opacity:1;} .sidebar-link{transition:all 0.15s ease;} .sidebar-link:hover{background:rgba(255,255,255,0.12)!important;padding-left:18px!important;} table.ev-table{width:100%;border-collapse:collapse;font-size:13.5px;} table.ev-table thead th{background:#f1f5f9;color:#475569;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.6px;padding:12px 14px;text-align:left;border-bottom:2px solid #e2e8f0;white-space:nowrap;} table.ev-table tbody td{padding:12px 14px;border-bottom:1px solid #f1f5f9;vertical-align:middle;} table.ev-table tbody tr:hover td{background:#f5f7ff;} table.ev-table tbody tr:last-child td{border-bottom:none;} .chip{display:inline-flex;align-items:center;gap:4px;padding:2px 8px;border-radius:20px;font-size:10.5px;font-weight:700;white-space:nowrap;} .chip-wk{background:#EBF0FA;color:${NAVY};} .chip-wwk{background:#ecfdf5;color:#065f46;} .badge-pill{display:inline-block;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:700;white-space:nowrap;} .form-section{background:white;border-radius:14px;padding:24px 28px;box-shadow:0 1px 12px rgba(0,0,0,0.06);border:1px solid #e8ecf0;margin-bottom:16px;} .form-section h3{margin:0 0 16px;font-size:15px;font-weight:800;color:${NAVY};} .stat-card{border-radius:12px;padding:16px 18px;display:flex;align-items:center;gap:12;} .modal-overlay{position:fixed;inset:0;z-index:8100;background:rgba(11,37,69,0.55);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:16px;animation:fadeIn 0.15s ease;} .modal-box{background:white;border-radius:18px;width:100%;max-height:92vh;display:flex;flex-direction:column;box-shadow:0 24px 60px rgba(0,0,0,0.2);animation:up 0.2s ease;} input:focus,select:focus,textarea:focus{outline:none;border-color:${NAVY}!important;box-shadow:0 0 0 3px rgba(11,37,69,0.08);} button:focus-visible{outline:2px solid ${GOLD};outline-offset:2px;}`;
