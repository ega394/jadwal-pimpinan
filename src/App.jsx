...
const mkEv=o=>({alur:"disetujui",catatanTolak:"",catatanKasubbag:"",catatanKabag:"",statusWK:null,statusWWK:null,perwakilanWK:"",perwakilanWWK:"",delegasiKeWWK:false,delegasiWWKJajaran:false,sambutanFile:null,sambutanNama:"",sambutanDocx:null,sambutanDocxNama:"",undanganFile:null,undanganNama:"",catatanPimpinan:"",tersembunyi:false,alurHapus:null,lokasi:"",personil:[],catatanPenugasan:"",evaluasi:{},...o});
const seed=[
...
function ReportingModal({events,onClose,kabagNama,cetakOleh}){
  ...
  const printPDF=()=>{
    ...
    let nomor=0;
    function hadirOleh(ev){var list=[];var forWK=(ev.untukPimpinan||[]).includes("walikota");var forWWK=(ev.untukPimpinan||[]).includes("wakilwalikota")||ev.delegasiKeWWK;if(forWK&&!ev.delegasiKeWWK){if(ev.statusWK==="hadir")list.push("Wali Kota");else if(ev.statusWK==="diwakilkan"&&ev.perwakilanWK)list.push(ev.perwakilanWK);else if(ev.statusWK==="diwakilkan")list.push("Perwakilan WK");}if(forWWK){if(ev.statusWWK==="hadir")list.push("Wakil Wali Kota");else if(ev.statusWWK==="diwakilkan"&&ev.perwakilanWWK)list.push(ev.perwakilanWWK);else if(ev.statusWWK==="diwakilkan")list.push("Perwakilan WWK");}return list.join(", ");}
...
const printF4L=()=>{
    ...
    const rows=Object.keys(byTgl).sort().flatMap(tgl=>{
      ...
      return evs.map((ev,iG)=>{
        nomor++;
function hadirOleh(ev){var list=[];var fWK=(ev.untukPimpinan||[]).includes("walikota");var fWWK=(ev.untukPimpinan||[]).includes("wakilwalikota")||ev.delegasiKeWWK;if(fWK&&!ev.delegasiKeWWK){if(ev.statusWK==="hadir")list.push("Wali Kota");else if(ev.statusWK==="diwakilkan"&&ev.perwakilanWK)list.push(ev.perwakilanWK);else if(ev.statusWK==="diwakilkan")list.push("Perwakilan WK");}if(fWWK){if(ev.statusWWK==="hadir")list.push("Wakil Wali Kota");else if(ev.statusWWK==="diwakilkan"&&ev.perwakilanWWK)list.push(ev.perwakilanWWK);else if(ev.statusWWK==="diwakilkan")list.push("Perwakilan WWK");}return list.join(", ");}
        ...
        return "<tr><td class='c'>"+nomor+"</td>"+tglCell+"<td class='c'><b>"+ev.jam+"</b></td><td><b>"+ev.namaAcara+"</b><br><span style='font-size:7.5pt;color:#64748b'>"+ev.penyelenggara+"</span></td><td>"+ev.lokasi+"</td><td style='font-size:8pt'>"+pkn+"</td><td style='font-size:8pt'>"+hadirOleh(ev)+"</td><td class='cat'></td></tr>";
      });
    }).join("");
    const w=window.open("","_blank");
...
function LaporanModal({events,onClose,kabagNama,cetakOleh}){
  ...
  const printPDF=()=>{
    ...
    const printDateTime=printDate+" pukul "+printTime+" WITA";
    const kabag=kabagNama||"Kabag Protokol & Komunikasi Pimpinan";
    const rows=Object.keys(byDay).sort().flatMap(tgl=>byDay[tgl].map((ev,i)=>"<tr>"+(i===0?"<td class='c' rowspan='"+byDay[tgl].length+"' style='background:#EBF0FA;font-weight:700;color:#0B2545'>"+getHari(tgl)+"<br><span style='font-size:7pt'>"+fmtShort(tgl)+"</span></td>":"")+"<td class='c'><strong>"+ev.jam+" WITA</strong></td><td><strong>"+ev.namaAcara+"</strong><br><span style='font-size:7.5pt;color:#64748b'>"+ev.penyelenggara+"</span></td><td class='c "+(ev.jenisKegiatan==="Sambutan"?"u":ev.jenisKegiatan==="Pengarahan"?"b":"g")+"'>"+ev.jenisKegiatan+"</td><td>"+(ev.lokasi||"<em style='color:#cbd5e1'>-</em>")+"</td><td class='c' style='font-size:7pt'>"+ev.pakaian+"</td><td class='c'>"+(function(e){var l=[];var wk=(e.untukPimpinan||[]).includes("walikota");var wwk=(e.untukPimpinan||[]).includes("wakilwalikota")||e.delegasiKeWWK;if(wk&&!e.delegasiKeWWK){if(e.statusWK==="hadir")l.push("Wali Kota");else if(e.statusWK==="diwakilkan"&&e.perwakilanWK)l.push(e.perwakilanWK);else if(e.statusWK==="diwakilkan")l.push("Perwakilan WK");}if(wwk){if(e.statusWWK==="hadir")l.push("Wakil Wali Kota");else if(e.statusWWK==="diwakilkan"&&e.perwakilanWWK)l.push(e.perwakilanWWK);else if(e.statusWWK==="diwakilkan")l.push("Perwakilan WWK");}return l.join(", ");})(ev)+"</td></tr>")).join("");
...
const printF4L_lap=()=>{
    ...
    const printDateTime=printDate+" pukul "+printTime+" WITA";
    const kabag=kabagNama||"Kabag Protokol & Komunikasi Pimpinan";
    const rows=Object.keys(byDay).sort().flatMap(tgl=>byDay[tgl].map((ev,i)=>"<tr>"+(i===0?"<td class='c' rowspan='"+byDay[tgl].length+"' style='background:#EBF0FA;font-weight:700;color:#0B2545'>"+getHari(tgl)+"<br><span style='font-size:7pt'>"+fmtShort(tgl)+"</span></td>":"")+"<td class='c'><strong>"+ev.jam+" WITA</strong></td><td><strong>"+ev.namaAcara+"</strong><br><span style='font-size:7.5pt;color:#64748b'>"+ev.penyelenggara+"</span></td><td class='c "+(ev.jenisKegiatan==="Sambutan"?"u":ev.jenisKegiatan==="Pengarahan"?"b":"g")+"'>"+ev.jenisKegiatan+"</td><td>"+(ev.lokasi||"<em style='color:#cbd5e1'>-</em>")+"</td><td class='c' style='font-size:7pt'>"+ev.pakaian+"</td><td class='c'>"+(function(e){var l=[];var wk=(e.untukPimpinan||[]).includes("walikota");var wwk=(e.untukPimpinan||[]).includes("wakilwalikota")||e.delegasiKeWWK;if(wk&&!e.delegasiKeWWK){if(e.statusWK==="hadir")l.push("Wali Kota");else if(e.statusWK==="diwakilkan"&&e.perwakilanWK)l.push(e.perwakilanWK);}if(wwk){if(e.statusWWK==="hadir")l.push("Wakil Wali Kota");else if(e.statusWWK==="diwakilkan"&&e.perwakilanWWK)l.push(e.perwakilanWWK);}return l.join(", ");})(ev)+"</td><td class='cat'></td></tr>")).join("");
    const statRow="<div style='display:flex;gap:10px;margin:8px 0;flex-wrap:wrap'>"+[["Total",stats.total,"#0B2545"],["Wali Kota",stats.wk,"#1B4080"],["Wakil WK",stats.wwk,"#065f46"],["Sambutan",stats.sambutan,"#7c3aed"],["Pengarahan",stats.pengarahan,"#2563eb"],["Menghadiri",stats.menghadiri,"#16a34a"]].map(([l,v,c])=>"<div style='background:"+c+";color:white;border-radius:8px;padding:6px 12px;font-size:10pt;font-weight:700;text-align:center'><div style='font-size:8pt;font-weight:400;opacity:0.8'>"+l+"</div>"+v+"</div>").join("")+"</div>";
  ...
  return <div style={{padding:isMobile?"12px":"20px",maxWidth:700,margin:"0 auto"}}>
    ...
    {pending.map(ev=><div key={ev.id} style={{background:"white",borderRadius:14,padding:"14px 16px",marginBottom:14,boxShadow:"0 2px 8px rgba(0,0,0,0.07)",border:"1.5px solid #fde68a"}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        ...
        {!isKasubbag&&<button onClick={()=>{upd(ev.id,{alur:"disetujui"});showT("Jadwal disetujui & tayang!","ok");const _u=loadUsers().find(u=>u.username===ev.submittedBy);if(_u?.noWA)sendWA({to:_u.noWA,namaAcara:ev.namaAcara,tanggal:ev.tanggal,jam:ev.jam,penyelenggara:ev.penyelenggara,lokasi:ev.lokasi,event:"approved",submittedBy:getNamaByUsername(ev.submittedBy)});sendPush({targetRole:"admin_rk",title:"✅ Jadwal Disetujui",body:ev.namaAcara+" sudah dipublikasi",url:"/",tag:"approved-"+ev.id});}} style={{padding:"8px 16px",borderRadius:8,border:"none",background:"#16a34a",color:"white",cursor:"pointer",fontSize:12,fontWeight:700}}>✅ Setujui & Publikasi</button>}
      </div>
    </div>)}
          ...
          {forWWK&&<div>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              <StatusBtn label="✓ Hadir" active={ev.statusWWK==="hadir"} color={GREEN}
                onClick={()=>{upd(ev.id,{statusWWK:"hadir",statusWWK_by:"ajudan",delegasiWWKJajaran:false,perwakilanWWK:""});showT("Kehadiran WWK diinput");}}/>
              <StatusBtn label="✗ Tidak Hadir" active={ev.statusWWK==="tidak_hadir"} color="#991b1b"
                onClick={()=>{upd(ev.id,{statusWWK:"tidak_hadir",statusWWK_by:"ajudan",delegasiWWKJajaran:false});showT("WWK: Tidak Hadir");}}/>
      ...
      <div style={{background:"white",borderRadius:14,marginBottom:10,overflow:"hidden",border:"1.5px solid #DBEAFE",boxShadow:"0 2px 10px rgba(10,22,40,0.07)"}}>
        ...
        {exp&&<div style={{borderTop:"1px solid #EFF6FF",padding:"12px 16px",background:"#FAFBFF"}}>
          ...
          <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:8}}>
            <button onClick={()=>{upd(ev.id,{alur:"disetujui"});showT("Jadwal disetujui & dipublikasi");const u=loadUsers().find(x=>x.username===ev.submittedBy);if(u?.noWA)sendWA({to:u.noWA,namaAcara:ev.namaAcara,tanggal:ev.tanggal,jam:ev.jam,penyelenggara:ev.penyelenggara,lokasi:ev.lokasi,event:"approved",submittedBy:getNamaByUsername(ev.submittedBy)});sendPush({targetRole:"admin_rk",title:"✅ Jadwal Disetujui",body:ev.namaAcara+" sudah dipublikasi",url:"/",tag:"approved-"+ev.id});setExpanded(null);}}
...
// ==================== FORM ====================
  ...
  const EventCard=({ev})=>{
    ...
    return <div id={"ev-"+ev.id} className="ev-card" style={{background:cardBg,borderRadius:16,marginBottom:10,boxShadow:"0 2px 12px rgba(0,0,0,0.07),0 0 0 1px rgba(0,0,0,0.04)",border:"1.5px solid "+borderColor,overflow:"hidden",opacity:cardOpacity}}>
      ...
      {["kabag","ajudan_walikota","ajudan_wakilwalikota","kasubbag_protokol","kasubbag_komdokpim","timkom","staf","admin_rk"].includes(role)&&ev.alur==="disetujui"&&(()=>{
        const all=loadUsers();
        ...
        return(
          <div style={{background:isAssigned?"#F0FDF4":urgent?"#FFF7ED":showNames?"#F0F9FF":"#F8FAFC",padding:showNames?"6px 14px":"5px 14px",borderBottom:"1px solid "+(isAssigned?"#BBF7D0":urgent?"#FED7AA":showNames?"#BAE6FD":"#E2E8F0"),display:"flex",alignItems:"flex-start",gap:8,flexWrap:"wrap"}}>
            ...
            {isAssigned&&ev.alur==="disetujui"&&(()=>{
              const sudahEval=!!(ev.evaluasi||{})[user.username]?.submitted;
              const sudahLewat=new Date()>new Date(ev.tanggal+"T"+ev.jam);
              if(!sudahLewat)return null;
              return <button onClick={e=>{e.stopPropagation();setEvaluasiEv(ev);}} style={{flexShrink:0,padding:"2px 8px",borderRadius:5,border:"1px solid "+(sudahEval?"#16A34A":"#7C3AED"),background:"white",color:sudahEval?"#16A34A":"#7C3AED",cursor:"pointer",fontSize:10,fontWeight:700}}>
                {sudahEval?"✅ Evaluasi":"📝 Evaluasi"}
              </button>;
            })()}
          </div>
        );
      ...
      {role==="kabag"&&<div style={{display:"flex",flexDirection:"column",gap:7}}>
        {ev.alur==="menunggu_kabag"&&!ev.alurHapus&&<>
          ...
          <button onClick={()=>{upd(ev.id,{alur:"disetujui"});showT("Jadwal disetujui & dipublikasi");
            {const _u=loadUsers().find(u=>u.username===ev.submittedBy);if(_u?.noWA)sendWA({to:_u.noWA,namaAcara:ev.namaAcara,tanggal:ev.tanggal,jam:ev.jam,penyelenggara:ev.penyelenggara,lokasi:ev.lokasi,event:"approved",submittedBy:getNamaByUsername(ev.submittedBy)});}sendPush({targetRole:"admin_rk",title:"✅ Jadwal Disetujui",body:ev.namaAcara+" sudah dipublikasi",url:"/",tag:"approved-"+ev.id});}}
